...
###V.5-6
##add additional enhancer count (non-promoter enahncer and inclusive enhancer)
## add TAD limited linear rescue
usage = (
"""
    ...
    TAD limited linear proximity-based enhancer rescue:
    EO is assigned to its nearest neighbor within the same TAD
...
def update_network(file,outfile,gene2enh_prom,gene2enhLoad_prom,gene2enh_genebody,gene2enhLoad_genebody,gene2enh_rest,gene2enhLoad_rest,gene2enh_TAD,gene2enhLoad_TAD,enh2prom,enhDist,gene2exp):
    """
    ...
    while True:
        ...
        med1_comm_E=int(cols[30])
        med1_comm=int(cols[31])

        community_order=int(cols[24].split(",")[1])+int(cols[25])
        enhs=cols[32]
        genes=cols[33]
        promLoc=cols[34]
        ...
        if gene not in gene2enh_TAD:
            gene2enh_TAD[gene]=["NA"]
            gene2enhLoad_TAD[gene]=0
        total_signal=med1_comm_E+gene2enhLoad_prom[gene]+gene2enhLoad_genebody[gene]+gene2enhLoad_rest[gene]
        essential_total_signal=med1_comm_E+gene2enhLoad_prom[gene]
        TAD_comm_E_signal=med1_comm_E+gene2enhLoad_TAD[gene]
        TAD_comm_signal=med1_comm+gene2enhLoad_TAD[gene]
        of.write(line.strip()+"\t"+"\t".join(map (str,
            (",".join(gene2enh_prom[gene]),gene2enhLoad_prom[gene],",".join(gene2enh_genebody[gene]),gene2enhLoad_genebody[gene],",".join(gene2enh_rest[gene]),gene2enhLoad_rest[gene],total_signal,essential_total_signal,",".join(closestGene),",".join(map(str,distToClosest)),",".join(map(str,distToFocal)),average_distToClosest,average_distToFocal,community_order,exp,",".join(gene2enh_TAD[gene]),gene2enhLoad_TAD[gene],TAD_comm_E_signal,TAD_comm_signal)))+"\n")
    f.close()
    of.close()
...
def main():
    ...
    ap.add_argument(
        ...
        "--TAD",
        dest="TADPromProx",
        help="enhancers missed by community (the rest) assigned by TAD limited promoter proximity"
    )
    ap.add_argument(
        "-e",
        "--enhs",
    ...
    if (not args.network) or (not args.promProx) or ( not args.geneBodyProx) or (not args.restPromProx) or (not args.enhs) or (not args.exp) or (not args.TADPromProx):
        ap.print_help()
        sys.exit(1)
    ...
    gene2enh_TAD,gene2enhLoad_TAD=map_enhancer(args.TADPromProx,float('inf'))
    enh2prom,enhDist=locate_enhancer(args.enhs)
    gene2exp=load_exp(args.exp)
    outfile=args.network+".update"
    update_network(args.network,outfile,gene2enh_prom,gene2enhLoad_prom,gene2enh_genebody,gene2enhLoad_genebody,gene2enh_rest,gene2enhLoad_rest,gene2enh_TAD,gene2enhLoad_TAD,enh2prom,enhDist,gene2exp)
